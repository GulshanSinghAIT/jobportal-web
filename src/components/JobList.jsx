...
export default function JobList({ jobs = [] }) {
  const { getToken } = useAuth(); // Clerk hook for getting token
  ...
  // Helper function to fetch the token and set headers
  ...
  useEffect(() => {
    async function loadSaved() {
      try {
        ...
        const res = await fetch(`${BASE_URL}/api/user/saved`, { headers });
    ...
    <>
      {userId ? (
                ...
                >
                  <Card className="h-full flex flex-col">
                    <div className="relative pt-12">
                      ...
                      >
                        <Button variant={isSaved ? "secondary" : "ghost"} size="icon">
                          {isSaving ? (
                    ...
                    <CardFooter className="flex justify-between gap-2 pt-2 pb-4">
                      <a href={job.job_link} className="flex-1" target="_blank" rel="noopener noreferrer">
        ...
        <div className="flex flex-col justify-center items-center py-12">
          <Briefcase className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
          <p className="text-lg text-muted-foreground">Please log in to view job listings.</p>
        </div>
      )}
    </>
  );
